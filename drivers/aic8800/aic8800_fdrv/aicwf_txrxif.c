...
int aicwf_bus_init(uint bus_hdrlen, struct device *dev)
{
	...
	AICWFDBG(LOGINFO, "%s rx_state %d, tx_state %d \r\n", __func__, bus_if->busrx_thread->__state, bus_if->bustx_thread->__state);
	#else
	AICWFDBG(LOGINFO, "%s rx_state %ld, tx_state %ld \r\n", __func__, bus_if->busrx_thread->state, bus_if->bustx_thread->state);
	#endif
#if 1
	...
	#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 14, 0)
	AICWFDBG(LOGINFO, "%s state %d %d \r\n", __func__, bus_if->busrx_thread->__state, bus_if->bustx_thread->__state);
	#else
    AICWFDBG(LOGINFO, "%s state %ld %ld \r\n", __func__, bus_if->busrx_thread->state, bus_if->bustx_thread->state);
	#endif
